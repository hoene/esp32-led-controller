...
static void getMessage() {
	...
	mbedtls_md_init(&ctx);
	ESP_ERROR_CHECK(
			mbedtls_md_setup(&ctx, mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),
					0));
...
{
	int res = send(home_socket, &home_message, sizeof(home_message), MSG_DONTWAIT);
...
static void task(void *args) {
	...
	vTaskDelay(configTICK_RATE_HZ * 60 + rand() % (configTICK_RATE_HZ * 60)); // wait for 60 to 120 seconds
	...
	for (;;) {
		...
		int incrementTicks = xTaskGetTickCount() + configTICK_RATE_HZ * 60 * 12 + rand() % (configTICK_RATE_HZ * 60 * 6) ; // every 12 to 18 minutes
...
void home_on() {
  ...
  ESP_ERROR_CHECK(xTaskCreate(task, "home", 2048, NULL, tskIDLE_PRIORITY + 1,
                              &taskHandle) == pdPASS
                      ? ESP_OK
                      : ESP_FAIL);
}
