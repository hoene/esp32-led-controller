...
esp_err_t get_handler_jpeg(httpd_req_t *req) {
  struct MJPEG_FILE *file = mjpeg_frame_access(500 * portTICK_PERIOD_MS);
  if (!file) {
    ESP_LOGI(TAG, "JPEG none");
    return httpd_resp_send_404(req);
  }

  ESP_LOGI(TAG, "send jpg %d %p", file->size, file->buffer);
...
const char *mimetypes[] = {".css", "text/css",      ".js",  "text/javascript",
...
esp_err_t get_handler(httpd_req_t *req) {
  ...
  if (len > 1) {
    for (struct HTML_FILE *p = html_files; p->data != NULL; p++) {
      if (!memcmp((void *)req->uri + 1, (void *)p->name, len - 1) &&
          !memcmp((void *)p->name + len - 1, (void *)".gz", 4)) {
        if (len > 4) {
  ...
  if (web_index_html != NULL && !strcmp(req->uri, "/")) {
    httpd_resp_set_hdr(req, "Content-Encoding", "gzip");
    return httpd_resp_send(req, (const char *)web_index_html->data,
                           *web_index_html->size);
  }
  return httpd_resp_send_404(req);
}
...
esp_err_t OTA_reboot_post_handler(httpd_req_t *req) {
  ...
  if ((recv_len = httpd_req_recv(req, ota_buff,
                                 content_length < sizeof(ota_buff)
                                     ? content_length
                                     : sizeof(ota_buff))) < 0) {
...
esp_err_t OTA_update_post_handler(httpd_req_t *req) {
  ...
  do {
    ...
    //content_length, ota_buff, ota_buff);
...
static const httpd_uri_t uri_get_jpeg = {.uri = "/rtp.jpg",
                                         .method = HTTP_GET,
                                         .handler = get_handler_jpeg,
                                         .user_ctx = NULL};

static const httpd_uri_t OTA_update = {.uri = "/upload",
                                       .method = HTTP_POST,
                                       .handler = OTA_update_post_handler,
                                       .user_ctx = NULL};

static const httpd_uri_t OTA_reboot = {.uri = "/reboot",
                                       .method = HTTP_POST,
